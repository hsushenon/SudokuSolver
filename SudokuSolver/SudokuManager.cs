namespace SudokuSolver
{
    public static class SudokuManager
    {
        ...
        ///Check if the number can be set in that column and row
        public static int IsValid(int r, int c, int no)
        {
        ...
        ///If you want to see each step of result set ch = 1. Set ch = 0 for only the final result.
        ///Currently ch = 1 does not work
        public static int[,] GenerateSolution(int ch)
        {
            int flag1 = 0, flag2 = 1, rowst, colmn_st, l2_k, i1, l2_j;
            int l2_i, l3_j, l3_i, l3_k1, l3_j1, c = 0, co = 0, j, k, i, count = 0, no = 0, n = 0, countp = 0;
            int[] po = new int[9];
                    ...
                    for (j = 0; j < 9; j++)
                        for (k = 0; k < 9; k++)
                        {
                            ...
                            for (i = 1; i < 10; i++)
                            {
                                ...
                                if (count == 2)//more than one no. is IsValid 
                                    break;
                            }

                            //logic 1.    for every cell check if only one no. is IsValid.. 
                            if (count == 1)
                            {
                                flag1 = 1;
                                ar[j, k] = no;
                            ...
                            /* logic 2  ..to test the choices. if one of the choice isnt repeated it is entered into the cell..
                             suppose positon(2,3) has 2,3,4 as IsValid chioce. if the no. 2 is not a choice in the (other cell of)2nd row
                            no. 2 has to be in position(2,3)...  similarly check for other choices if 2 is repeated. also check in 3rd column,
                            and the grid similarly.
                            */
        ...
        public static bool SetSudoku(int r, int c, int no)
        {
            if (no < 1 && no > 9)
                return false;
